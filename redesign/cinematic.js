(()=>{
 ...
 const img=new Image();img.decoding='async';img.src='images/visioncraft-official-logo.webp';let t=0,raf=0,visible=true;
 ...
 const obs=new IntersectionObserver(es=>{visible=es[0]?.isIntersecting;if(visible&&!raf&&img.complete&&img.naturalWidth)draw()});obs.observe(canvas);
 function draw(){raf=0;if(!visible||!img.complete||!img.naturalWidth)return;const w=canvas.clientWidth,h=canvas.clientHeight;ctx.clearRect(0,0,w,h);t+=.008;const iw=Math.min(w*.72,680),ih=iw*(img.naturalHeight/img.naturalWidth),x=w*.5-iw*.5+Math.sin(t)*9,y=h*.5-ih*.5+Math.cos(t*.8)*12;ctx.save();ctx.translate(w*.5,h*.5);ctx.rotate(Math.sin(t*.45)*.015);ctx.translate(-w*.5,-h*.5);ctx.shadowColor='rgba(0,0,0,.24)';ctx.shadowBlur=45;ctx.shadowOffsetY=30;ctx.drawImage(img,x,y,iw,ih);ctx.restore();const g=ctx.createRadialGradient(w*.62,h*.38,0,w*.62,h*.38,w*.36);g.addColorStop(0,'rgba(255,174,132,.12)');g.addColorStop(.55,'rgba(101,74,190,.05)');g.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=g;ctx.fillRect(0,0,w,h);raf=requestAnimationFrame(draw)}
 img.onload=()=>{document.documentElement.classList.add('vc-webgl');draw();};
 img.onerror=()=>{document.documentElement.classList.remove('vc-webgl');};
})();
